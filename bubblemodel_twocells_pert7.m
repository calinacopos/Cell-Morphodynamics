...
%         end
    ...
    %     b1 = acos(t2/t1*cos(b2));
        ...
        % (include tiny triangles in area calculation, works 6/16/21)
        tt1 = 0.5*( cos(b1)-cos(a1)*tan(b1) )*( sin(b1)-cos(a1)*tan(b1)*tan(b1) );
        tt2 = 0.5*( cos(b2)-cos(a2)*tan(b2) )*( sin(b2)-cos(a2)*tan(b2)*tan(b2) );
        r1 = ( (a1+b1) - 0.5*sin(a1)*cos(a1) - 0.5*cos(a1)*cos(a1)*tan(b1) + tt1)^(-1/2);
        r2 = ( (a2+b2) - 0.5*sin(a2)*cos(a2) - 0.5*cos(a2)*cos(a2)*tan(b2) + tt2)^(-1/2);
