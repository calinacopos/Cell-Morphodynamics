...
tL  = 1.0;          % leader force
tT  = 1.25;          % trailer force
tcc = 0.5;%0.9;          % cell-cell force
...
if(aL>pi/2)
    x1 = cL(1) + rL*cos(aL-pi/2);
    y1 = cL(2) - rL*sin(aL-pi/2);
else 
    x1 = cL(1) + rL*cos(aL);
    y1 = cL(2) + rL*sin(aL);
end
...
if(aT>pi/2)
    x3 = cT(1) - rT*cos(aT-pi/2);
    y3 = cT(2) - rT*sin(aT-pi/2);
else
    x3 = cT(1) - rT*cos(aT);
    y3 = cT(2) + rT*sin(aT);
end
...
% determinable (?)
h = 2*polyarea([x1,x3,x4],[y1,y3,y4])/sqrt((x1-x3)^2+(y1-y3)^2);
...
% tilt between x1 and x3
%phi = asin(abs(y1-y3)/sqrt((x1-x3)^2+(y1-y3)^2));

%x2 = x4 + h*sin(phi+w)/cos(w);
%y2 = y4 - h*cos(phi+w)/cos(w);
...
vidObj = VideoWriter('simulation_fp0_frneg0dot2.mp4','MPEG-4');
open(vidObj);
for t=1:round(Tend/dt)
    ...
    if(aT>pi/2)
        th2 = linspace(pi/2-bT,pi/2+aT,100);
        %th2 = linspace(aT,pi/2-bT,100);
    else
        %th2 = linspace(aT-pi/2,pi/2-bT,100);
        th2 = linspace(pi/2-bT,pi/2+aT,100);
    end
    ...
    xLarea = [cL(1)+rL*cos(th1),cC(1)+rcc*cos(fliplr(th3)),linspace(x2,x1,100)];
    yLarea = [cL(2)+rL*sin(th1),cC(2)+rcc*sin(fliplr(th3)),linspace(y2,y1,100)];
    %fill(xLarea,yLarea,[0.7 0 0.5],'facealpha',0.2,'edgecolor','none');
    
    %xTarea = [linspace(x2,x3,100),cT(1)+rT*cos(th2),cC(1)+rcc*cos(th3)];
    %yTarea = [linspace(y2,y3,100),cT(2)+rT*sin(th2),cC(2)+rcc*sin(th3)];
    xTarea = [cC(1)+rcc*cos(th3),cT(1)+rT*cos(th2),linspace(x3,x2,100)];
    yTarea = [cC(2)+rcc*sin(th3),cT(2)+rT*sin(th2),linspace(y3,y2,100)];
...
%     end
    ...
    if mod(t,outevery) == 0
        ...
        scatter(x4,y4,100,'ko','fill','k')
         ...
         % forces
        quiver(x1,y1,0.0,0,'-k','linewidth',1);
        ...
        quiver(x4,y4,v4x,v4y,'-','color',[0.75 0.75 0.75],'linewidth',3,'maxheadsize',10);
